...
namespace CmPayments\Crate\Command;

use CmPayments\Crate\Crate;
use CmPayments\Crate\StubGenerator;
use CmPayments\Crate\Configuration;
...
<comment>
  ...
  the following files will be used (in order): <info>crate.json,
  crate.json.dist</info>
</comment>
...
The following compactors are included with Crate:
<comment>
...
unmodified.
<comment>
It may be useful to know that Crate imports files in the following order:
...
HELP
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->crate = Crate::create($path);
        $this->crate->getPhar()->startBuffering();
        ...
        if (true === $this->config->isStubGenerated()) {
            ...
            $this->crate->getPhar()->setStub($stub->generate());
            ...
            $this->crate->setStubUsingFile($stub);
        } else {
        ...
        if (null !== ($metadata = $this->config->getMetadata())) {
            ...
            $this->crate->getPhar()->setMetadata($metadata);
        }
        ...
        if (null !== ($algorithm = $this->config->getCompressionAlgorithm())) {
            ...
            $this->crate->getPhar()->compressFiles($algorithm);
        }

        $this->crate->getPhar()->stopBuffering();
